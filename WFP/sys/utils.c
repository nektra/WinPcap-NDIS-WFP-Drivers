...
Environment:
	...
	IMPORTANTE: CADA NET BUFFER ES UN PAQUETE. NET BUFFER LIST ES UNA LISTA DE PAQUETES "RELACIONADOS"
...
BOOLEAN IsAleReauthorize(_In_ const FWPS_INCOMING_VALUES* inFixedValues)
{
   UINT flagsIndex;

   GetFlagsIndexesForLayer(inFixedValues->layerId, &flagsIndex );
...
BOOLEAN IsSecureConnection( _In_ const FWPS_INCOMING_VALUES* inFixedValues)
{
   UINT flagsIndex;
...
BOOLEAN IsAleClassifyRequired(_In_ const FWPS_INCOMING_VALUES* inFixedValues,_In_ const FWPS_INCOMING_METADATA_VALUES* inMetaValues)
{
   //
...
BOOLEAN IsMatchingConnectPacket(_In_ const FWPS_INCOMING_VALUES* inFixedValues, _In_ ADDRESS_FAMILY addressFamily, _In_ FWP_DIRECTION direction, _Inout_ TL_INSPECT_PENDED_PACKET* pendedPacket)
{
   UINT localAddrIndex;
   ...
   if (inFixedValues->incomingValue[protocolIndex].value.uint8 != pendedPacket->protocol)
   {
      return FALSE;
   }

   if (RtlUshortByteSwap(
         inFixedValues->incomingValue[localPortIndex].value.uint16) != pendedPacket->localPort)
   {
      return FALSE;
   }

   if (RtlUshortByteSwap(
         inFixedValues->incomingValue[remotePortIndex].value.uint16) != pendedPacket->remotePort)
   {
      return FALSE;
   }
   ...
   {
      UINT32 ipv4LocalAddr =  RtlUlongByteSwap(inFixedValues->incomingValue[localAddrIndex].value.uint32);
...
void FillNetwork5Tuple(_In_ const FWPS_INCOMING_VALUES* inFixedValues, _In_ ADDRESS_FAMILY addressFamily, _Inout_ TL_INSPECT_PENDED_PACKET* packet)
{
   UINT localAddrIndex;
   UINT remoteAddrIndex;
   UINT localPortIndex;
...
void FreePendedPacket( _Inout_ __drv_freesMem(Mem) TL_INSPECT_PENDED_PACKET* packet)
{
...
TL_INSPECT_PENDED_PACKET* AllocateAndInitializePendedPacket(
															...
															)
{
   ...
   pendedPacket = ExAllocatePoolWithTag(NonPagedPool, sizeof(TL_INSPECT_PENDED_PACKET), TL_INSPECT_PENDED_PACKET_POOL_TAG);
   ...
   FillNetwork5Tuple(inFixedValues, addressFamily, pendedPacket);
   ...
   NT_ASSERT(FWPS_IS_METADATA_FIELD_PRESENT(inMetaValues, FWPS_METADATA_FIELD_COMPARTMENT_ID));

   pendedPacket->compartmentId = inMetaValues->compartmentId;

   if ((pendedPacket->direction == FWP_DIRECTION_OUTBOUND) && (layerData != NULL))
   {
      NT_ASSERT(FWPS_IS_METADATA_FIELD_PRESENT(inMetaValues, FWPS_METADATA_FIELD_TRANSPORT_ENDPOINT_HANDLE));
      ...
      if (FWPS_IS_METADATA_FIELD_PRESENT(inMetaValues, FWPS_METADATA_FIELD_TRANSPORT_CONTROL_DATA))
      {
         ...
         pendedPacket->controlData = ExAllocatePoolWithTag(NonPagedPool, inMetaValues->controlDataLength, TL_INSPECT_CONTROL_DATA_POOL_TAG);
   ...
   else if (pendedPacket->direction == FWP_DIRECTION_INBOUND)
   {
      ...
      pendedPacket->interfaceIndex = inFixedValues->incomingValue[interfaceIndexIndex].value.uint32;
      pendedPacket->subInterfaceIndex = inFixedValues->incomingValue[subInterfaceIndexIndex].value.uint32;
      
      NT_ASSERT(FWPS_IS_METADATA_FIELD_PRESENT(inMetaValues, FWPS_METADATA_FIELD_IP_HEADER_SIZE));
      ...
      if (pendedPacket->netBufferList != NULL)
      {
         ...
         FwpsGetPacketListSecurityInformation(pendedPacket->netBufferList, FWPS_PACKET_LIST_INFORMATION_QUERY_IPSEC | FWPS_PACKET_LIST_INFORMATION_QUERY_INBOUND, &packetInfo);

         pendedPacket->ipSecProtected = (BOOLEAN)packetInfo.ipsecInformation.inbound.isSecure;

         pendedPacket->nblOffset =  NET_BUFFER_DATA_OFFSET(\
...
BOOLEAN IsTrafficPermitted(void)
{
   NTSTATUS status;
