...
-- */
{
   ...
   if (layerData != NULL)
   {
      ...
      //
      packetState = FwpsQueryPacketInjectionState(gInjectionHandle, layerData, NULL);
   ...
   if (!IsAleReauthorize(inFixedValues))
   {
      ...
      NT_ASSERT(FWPS_IS_METADATA_FIELD_PRESENT(inMetaValues, FWPS_METADATA_FIELD_COMPLETION_HANDLE));

      //
      // Pend the ALE_AUTH_CONNECT classify.
      //
      status = FwpsPendOperation(inMetaValues->completionHandle, &pendedConnect->completionContext);
      ...
      signalWorkerThread = IsListEmpty(&gConnList) && IsListEmpty(&gPacketQueue);
   ...
   else // re-auth @ ALE_AUTH_CONNECT
   {
      ...
      NT_ASSERT(FWPS_IS_METADATA_FIELD_PRESENT(inMetaValues, FWPS_METADATA_FIELD_PACKET_DIRECTION));
      ...
      if (packetDirection == FWP_DIRECTION_OUTBOUND)
      {
         ...
         for (listEntry = gConnList.Flink; listEntry != &gConnList; listEntry = listEntry->Flink)
         {
            connEntry = CONTAINING_RECORD(listEntry, TL_INSPECT_PENDED_PACKET, listEntry);

            if (IsMatchingConnectPacket(inFixedValues, addressFamily, packetDirection, connEntry) && (connEntry->authConnectDecision != 0))
            {
               // We found a match.
               pendedConnect = connEntry;

               NT_ASSERT((pendedConnect->authConnectDecision == FWP_ACTION_PERMIT) || (pendedConnect->authConnectDecision == FWP_ACTION_BLOCK));
               ...
               if (!gDriverUnloading && (pendedConnect->netBufferList != NULL) && (pendedConnect->authConnectDecision == FWP_ACTION_PERMIT))
               {
                  //
                  ...
                  signalWorkerThread = IsListEmpty(&gPacketQueue) && IsListEmpty(&gConnList);
...
-- */
{
  ...
  _Analysis_assume_(layerData != NULL);
   ...
   if (!IsAleReauthorize(inFixedValues))
   {
      ...
      //
      status = FwpsPendOperation(inMetaValues->completionHandle, &pendedRecvAccept->completionContext);
...
void TLInspectTransportClassify(
							    _In_ const FWPS_INCOMING_VALUES* inFixedValues,
							    _In_ const FWPS_INCOMING_METADATA_VALUES* inMetaValues,
							    _Inout_opt_ void* layerData,
							    _In_opt_ const void* classifyContext,
...
-- */
{
  ...
  _Analysis_assume_(layerData != NULL);
   ...
   pendedPacket = AllocateAndInitializePendedPacket(inFixedValues, inMetaValues, addressFamily, layerData, TL_INSPECT_DATA_PACKET, packetDirection);//PASO AL WORKER THREAD EL PAQUETE Y EL NBL
...
NTSTATUS TLInspectALEConnectNotify(_In_  FWPS_CALLOUT_NOTIFY_TYPE notifyType, _In_ const GUID* filterKey, _Inout_ const FWPS_FILTER* filter)
{
...
NTSTATUS TLInspectALERecvAcceptNotify(_In_ FWPS_CALLOUT_NOTIFY_TYPE notifyType, _In_ const GUID* filterKey, _Inout_ const FWPS_FILTER* filter)
{
   UNREFERENCED_PARAMETER(notifyType);
   UNREFERENCED_PARAMETER(filterKey);
   UNREFERENCED_PARAMETER(filter);
...
NTSTATUS TLInspectTransportNotify(_In_ FWPS_CALLOUT_NOTIFY_TYPE notifyType, _In_ const GUID* filterKey, _Inout_ const FWPS_FILTER* filter)
{
   UNREFERENCED_PARAMETER(notifyType);
   UNREFERENCED_PARAMETER(filterKey);
   UNREFERENCED_PARAMETER(filter);
...
void TLInspectInjectComplete(_Inout_ void* context, _Inout_ NET_BUFFER_LIST* netBufferList, _In_ BOOLEAN dispatchLevel)
{
...
NTSTATUS TLInspectCloneReinjectInbound( _Inout_ TL_INSPECT_PENDED_PACKET* packet)
/* ++
...
-- */
{
   ...
   if (packet->completionContext != NULL)
   {
      ...
      FwpsCompleteOperation(packet->completionContext, clonedNetBufferList);
...
void
TlInspectCompletePendedConnection( _Inout_ TL_INSPECT_PENDED_PACKET** pendedConnect, _In_ BOOLEAN permitTraffic)
/* ++
...
-- */
{
   ...
   if (pendedConnectLocal->direction == FWP_DIRECTION_OUTBOUND)
   {
      ...
      pendedConnectLocal->authConnectDecision = permitTraffic ? FWP_ACTION_PERMIT : FWP_ACTION_BLOCK;
      ...
      FwpsCompleteOperation(completionContext, NULL);
...
void TLInspectWorker(_In_ void* StartContext)
/* ++
...
-- */
{
   ...
   for(;;)
   {
      ...
      KeWaitForSingleObject(&gWorkerEvent, Executive, KernelMode, FALSE, NULL);
      ...
      KeAcquireInStackQueuedSpinLock(&gConnListLock, &connListLockHandle);
	  //Vamos a agarrar un paquete, primero nos fijamos si hay conexiones
      if (!IsListEmpty(&gConnList))
      {
         _Analysis_assume_(gConnList.Flink != NULL);
         listEntry = gConnList.Flink;

         packet = CONTAINING_RECORD(listEntry, TL_INSPECT_PENDED_PACKET, listEntry);
      ...
      if (listEntry == NULL)
      {//si no habia ninguna conexion
         NT_ASSERT(!IsListEmpty(&gPacketQueue));

         KeAcquireInStackQueuedSpinLock(&gPacketQueueLock, &packetQueueLockHandle);

         listEntry = RemoveHeadList(&gPacketQueue);//TOMO EL PRIMER ELEMENTO DE LA PACKETQUETE Y LO SACO DE LA PACKETQUEUE

         packet = CONTAINING_RECORD(listEntry, TL_INSPECT_PENDED_PACKET, listEntry);// TOMO EL PRIMER ELEMENTO POSTA DE TL_INSPEC_PENDED_PACKET
		 
         KeReleaseInStackQueuedSpinLock(&packetQueueLockHandle);
      }

	  
	  if (packet->type == TL_INSPECT_CONNECT_PACKET) //EL PACKET != NULL NO ES ORIGINAL
      {
         TlInspectCompletePendedConnection(&packet, configPermitTraffic);
      }
         ...
         else
         {
			 ...
			 if (nbl != NULL)
			 {
				 ...
				 if (nb != NULL)
				 {
					 ...
					 //DbgPrint("PacketSize: %lu ", packetSize);
					 //DbgPrint("Packet Count: %d \n", packetCount);
					 packetCount++;
				 }
			 }
